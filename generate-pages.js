...
.foot-spanky{color:var(--orange);text-decoration:none;font-weight:700}.foot-spanky:hover{text-decoration:underline}
...
.hstats{display:inline-flex;gap:0;margin-top:26px;flex-wrap:wrap;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.10);border-radius:14px;overflow:hidden}
.hstat{padding:15px 26px;border-right:1px solid rgba(255,255,255,.08)}
.hstat:last-child{border-right:none}
.hstat .n{font-family:'Space Grotesk',sans-serif;font-size:32px;font-weight:700;color:#fff;line-height:1;letter-spacing:-.03em}
.hstat .n.accent{color:var(--orange)}
.hstat.hero .n{font-size:40px}
.hstat .l{font-size:10.5px;color:#849ABC;margin-top:8px;font-weight:600;letter-spacing:.05em;text-transform:uppercase}
@media(max-width:520px){.hstat{padding:13px 18px}.hstat .n{font-size:26px}.hstat.hero .n{font-size:32px}}
...
.donate-btns{display:flex;gap:10px;flex-wrap:wrap;flex:none}
.donate-btn.alt{background:transparent;border:1.5px solid #ff9250;padding:10.5px 20px}
.donate-btns .donate-btn{box-sizing:border-box;text-align:center}
@media(max-width:560px){.donate-btns{width:100%;flex-wrap:nowrap}.donate-btns .donate-btn{flex:1 1 0;min-width:0;padding:9px 8px;font-size:13px}}
...
// Locals whose calls come from a transcribed PHONE JOB LINE (not the hall's own
...
const JOBLINE_LOCALS = new Set([10008, 10014, 10025, 10043, 10058, 10086, 10159, 10234, 10242, 10294, 10309, 10332, 10340, 10350, 10364, 10575, 10649, 10668, 10743, 11141, 10076, 10110, 10150, 10508, 10701, 10915, 10046, 10038, 10176, 10223, 10233]);
function joblineNote(localId, es, trade) {
  ...
  return `<div class="jobline-note">${es
    ...
    : '\u26A0\uFE0F <b>Transcribed from the local\u2019s phone job line.</b> Details may contain errors \u2014 always confirm with the hall before you roll.'}</div>`;
}
...
<p style="margin:10px 0"><a href="/privacy" style="color:#9FB3D0;text-decoration:underline">Privacy Policy</a> &nbsp;·&nbsp; <a href="/terms" style="color:#9FB3D0;text-decoration:underline">Terms of Service</a></p><p class="credit"><a href="https://linktr.ee/spankythesparky" target="_blank" rel="noopener" class="foot-spanky">${F.credit}</a></p><p class="copyright">© ${TODAY.getFullYear()} TrampHereBro. ${F.rights}</p></div></footer>`;
}
...
<div class="hstat hero"><div class="n accent">${hands}</div><div class="l">${H.hn}</div></div>
...
function boardDigest(rows) {
  ...
  const agg = withCalls.map(r => {
    ...
    return { label: 'LU-' + (n || r.local.id), place: [r.local.city, r.local.state].filter(Boolean).join(', '), calls: r.calls.length, hands, maxScale, scaleCount: scales.length, book1: r.local.book1, top };
  });
  const picked = new Map();
  agg.slice().sort((a, b) => b.hands - a.hands).slice(0, 14).forEach(x => picked.set(x.label, x));
  agg.slice().filter(x => x.scaleCount >= 2).sort((a, b) => b.maxScale - a.maxScale).slice(0, 4).forEach(x => picked.set(x.label, x));
  const digest = [...picked.values()].map(x => `${x.label} ${x.place} — ${x.calls} calls, ~${x.hands} hands, highest single call $${x.maxScale.toFixed(2)} (that is ONE call's rate, NOT the local's scale; only ${x.scaleCount} of ${x.calls} calls list a rate)${x.book1 != null ? ', Book1 ' + x.book1 : ''}. ${x.top}`).join('\n');
  return { digest, stats };
}
async function generateSnapshot(rows, trade) {
  ...
  const CFG = trade === 'LINEMAN'
    ? { title: 'Trampin the IBEW Lineman Locals', who: 'traveling outside linemen', proj: 'transmission lines, substations, distribution rebuilds, storm restoration, and utility/data-center work' }
    : trade === 'UA'
    ? { title: 'Trampin the UA Locals', who: 'traveling plumbers, pipefitters, steamfitters and welders', proj: 'data centers, refineries, power plants, hospitals, semiconductor fabs and industrial process piping' }
    : { title: 'Trampin the IBEW Indoor Locals', who: 'traveling inside wiremen', proj: 'data centers, refineries, steel mills, hospitals' };
  const prompt = `You are writing today's "${CFG.title}" for TrampHereBro — a daily intel briefing for ${CFG.who} deciding where to chase work. You know this trade cold. Today is ${dayname}, ${PRETTY_DATE}.\n\nBoard-wide right now: ${stats.totalCalls} open calls across ${stats.activeLocals} locals, about ${stats.totalHands} hands needed.\n\nStandout locals (live from union dispatch — each line gives the local, its total calls and hands, top scale, book depth, then specific calls with contractor, project, pay, per diem, schedule and requirements):\n${digest}\n\nWrite a punchy, SPECIFIC editorial snapshot of ~240-300 words in the voice of a sharp journeyman who's actually in the work — not a generic recap. Lead with a bold title line exactly: **${CFG.title} — ${dayname}, ${PRETTY_DATE}**. Then feature the 5-6 most notable locals as tight paragraphs, ordered by a mix of top pay and biggest boards. For EACH featured local, be concrete with the data: bold the local header like **LU-494 Milwaukee, WI**; ONLY IF a pay figure for that local appears in the data above, bold it exactly as written there (e.g. **$62.73/hr**) and call out over-scale premiums, per diem, and OT specifics (e.g. "all OT double time", "5-10s + Sat"); name the actual contractors and projects (${CFG.proj}). Where the data shows book depth, work it in (e.g. "Book 1 nearly clear at 5 out"). Skip locals with thin data. Close with ONE sentence on the market trend — where the hands are going and what's driving it. No preamble, no sign-off. HARD RULE ON SCALE: the 'highest single call' figure is the rate on ONE specific call at that local, often a specialty or certification job. It is NOT the local's scale and NOT what the other calls pay. Never write it as the local's rate, never pair it with the local's total call or hand count (e.g. never 'N calls at $X/hr'), and never imply the board pays it. If you cite it, tie it to that one call and its contractor. HARD RULE ON MONEY: every dollar amount, hourly rate, scale figure, per diem and premium you write MUST appear verbatim in the data above. Do not infer a wage from the region, the trade, the contractor, or general knowledge of IBEW scale. Do not round, average, or approximate. If you are not copying a number straight from the data, do not write a number at all. A local with no pay data is still worth featuring for its board size, contractors and projects - just say nothing about pay for it. Same rule for contractors and project names: use only what appears in the data.`;
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-api-key': ANTHROPIC_KEY, 'anthropic-version': '2023-06-01' },
...
function snapTradeBlock(text, textLine, textUA){
  var trades = [['IBEW','IBEW Inside Wiremen', text]];
  if (textLine) trades.push(['LINEMAN','IBEW Lineman', textLine]);
  if (textUA) trades.push(['UA','UA Plumbers & Pipefitters', textUA]);
...
<main class="wrap">
${snapTradeBlock(text, textLine, textUA)}
...
(async function main() {
  ...
  let snapText = null, snapTextLine = null, snapTextUA = null;
  ...
  const hUA = callsHash(rows.filter(r => r.local.trade === 'UA').flatMap(r => r.calls));
  if (snapCache.UA && snapCache.UA.hash === hUA) snapTextUA = snapCache.UA.text;
  ...
  if (!snapTextUA && ANTHROPIC_KEY) { snapTextUA = await generateSnapshot(rows, 'UA'); if (snapTextUA) { snapCache.UA = { hash: hUA, text: snapTextUA }; console.log('  generated UA snapshot via ' + SNAPSHOT_MODEL); } }
  ...
  if (snapText) { fs.writeFileSync(path.join(SITE_DIR, 'snapshot.html'), snapshotPage(snapText, snapTextLine, snapTextUA)); console.log('  wrote snapshot.html'); }
  const ES_DIR = path.join(SITE_DIR, 'es');
  if (!fs.existsSync(ES_DIR)) fs.mkdirSync(ES_DIR, { recursive: true });
  const BILINGUAL = [
